...
Nevertheless, it is optimized for the Linux operating system. We use Windows on our local machines, so here is a short guide for Windows users:

1. Have Python installed to create a virtual environment (`python -m venv .venv`).
...
This command requires having the python virtual environment activated on the root level and the VSS Tools installed. The output will be in the file `own_vss.json` which contains the built VSS based on our structure of the repository.
